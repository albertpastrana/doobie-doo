...
class DooSpec extends DbSpecification {
    ...
    "select companies as tuples" >> {
      ...
      "should return all the companies in the db sorted by name ASC" >> {
        Doo.companyTuples must beRight(
          be_===(List((CompanyId(2), "Another Company", None), (CompanyId(1), "Intent HQ", Some("https://www.intenthq.com"))))
        )
      }
  ...
  "other interesting things" >> {
    ...
    "asking for a single result if the query returns more than one row returns an error" >> {
      Doo.Q.companiesCaseClass.option.transact(dbContext.xa).attempt
        .unsafePerformIO must beLeft(UnexpectedContinuation)
      Doo.Q.companiesCaseClass.unique.transact(dbContext.xa).attempt
        .unsafePerformIO must beLeft(UnexpectedContinuation)
    }
    ...
    "check" >> {
      ...
      check(Doo.Q.insertCompany("name"))
      check(Doo.Q.updateCompanyName(CompanyId(1), "name"))
      check(Doo.Q.companiesCaseClass)
      check(Doo.Q.companiesTuple)
      check(Doo.Q.companyCaseClass(CompanyId(1)))
      check(Doo.Q.jobOffersTuples)
      check(Doo.Q.jobOffers)
      check(Doo.Q.insertJobOffer(CompanyId(1), "summary", "description"))
    }
  }
